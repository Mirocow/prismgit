...
.PHONY: tauri-build
tauri-build: ## Build production Tauri installer for current OS (app + dmg on macOS)
	@echo "$(COLOR_YELLOW)→ Building Tauri production package...$(COLOR_RESET)"
	@npm run build
	@npx tauri build || (echo "$(COLOR_YELLOW)⚠ Full build failed (likely .dmg bundling). Falling back to .app only...$(COLOR_RESET)" && npx tauri build --bundles app)
	@echo "$(COLOR_GREEN)✓ Tauri installer ready in src-tauri/target/release/bundle/$(COLOR_RESET)"

.PHONY: tauri-build-app
tauri-build-app: ## Build Tauri .app only (skips .dmg — avoids bundle_dmg.sh failures on macOS)
	@echo "$(COLOR_YELLOW)→ Building Tauri .app (no .dmg)...$(COLOR_RESET)"
	@npm run build
	@npx tauri build --bundles app
	@echo "$(COLOR_GREEN)✓ Tauri .app ready in src-tauri/target/release/bundle/macos/$(COLOR_RESET)"
